...
// additions for the package edlibR begin here:
...
Rcpp::List edlibalign(std::string query, std::string target, std::string input_mode, std::string input_task, 
    int k, std::string cigar_format, Rcpp::Nullable<Rcpp::List> input_additionalEqualities = R_NilValue) {

   Rcpp::List finalresult = Rcpp::List::create(Rcpp::Named("editDistance"), 
        Rcpp::Named("alphabetLength"),
        Rcpp::Named("locations"),
        Rcpp::Named("cigar"), 
        Rcpp::Named("cigarFormat"));

    EdlibAlignMode mode;
    if (input_mode == "NW") {
        mode = EDLIB_MODE_NW;
    } else if (input_mode == "HW") {
        mode = EDLIB_MODE_HW;
    } else if (input_mode == "SHW") {
        mode = EDLIB_MODE_SHW;
    } else {
        Rcpp::stop("Invalid 'mode' argument entered");
    }

    EdlibAlignTask task;
    if (input_task == "distance") {
        task = EDLIB_TASK_DISTANCE;
    } else if (input_task == "locations") {
        task = EDLIB_TASK_LOC;
    } else if (input_task == "path") {
        task = EDLIB_TASK_PATH;
    } else {
        Rcpp::stop("Invalid 'task' argument entered");
    }

    EdlibCigarFormat cigarFormat;
    if (cigar_format=="extended") {
        cigarFormat = EDLIB_CIGAR_EXTENDED;   //!< Match: '=', Insertion: 'I', Deletion: 'D', Mismatch: 'X'
    } else if (cigar_format=="standard") {
        cigarFormat = EDLIB_CIGAR_STANDARD;   //!< Match: 'M', Insertion: 'I', Deletion: 'D', Mismatch: 'M'
    } else {
        Rcpp::stop("Invalid 'cigarFormat' argument entered");
    }

    int additionalEqualitiesLength;
    EdlibEqualityPair* additionalEqualities;
    if (input_additionalEqualities.isNotNull()){
        Rcpp::List additionalEqualities_list(input_additionalEqualities);
        additionalEqualitiesLength = additionalEqualities_list.size();
        additionalEqualities = (EdlibEqualityPair*)malloc(additionalEqualitiesLength * sizeof(EdlibEqualityPair));
        for (int i = 0; i < additionalEqualitiesLength; ++i) {
            Rcpp::CharacterVector start_end = additionalEqualities_list[i]; 
            std::string start = static_cast<std::string>(start_end[0]);
            std::string end = static_cast<std::string>(start_end[1]); 
            additionalEqualities[i].first = start[0];
            additionalEqualities[i].second = end[0];
        }
    } else {
        // default behavior
    ...
    EdlibAlignConfig config = edlibNewAlignConfig(k, mode, task, additionalEqualities, additionalEqualitiesLength);

    // char* cquery = const_cast<char*>(query.c_str());   // queryOriginal
    // char* ctarget = const_cast<char*>(target.c_str()); // targetOriginal


    // cresult
    // EdlibAlignResult cresult = edlibAlign(cquery, strlen_utf8(cquery), ctarget, strlen_utf8(ctarget), config);
    EdlibAlignResult cresult = edlibAlign(query.c_str(), query.size(), target.c_str(), target.size(), config);

    if (cresult.status == 1) {
        Rcpp::stop("There was an error using align()");
    }

    finalresult["editDistance"] = cresult.editDistance;
    finalresult["alphabetLength"] = cresult.alphabetLength; 
    // return cigar_format for the R function getNiceAlignment()
    finalresult["cigarFormat"] = cigar_format;   

    char* cigar_result = edlibAlignmentToCigar(cresult.alignment, cresult.alignmentLength, cigarFormat); 

    std::string cigar_string(cigar_result);

    if (cigar_string.empty()) {
        finalresult["cigar"] = R_NilValue;
    } else {
        finalresult["cigar"] =  cigar_string;
    }
    
    Rcpp::List locations;
    Rcpp::IntegerVector start_end;
    if (cresult.numLocations > 0) {
        for (int i = 0; i < cresult.numLocations; ++i) {
            if (cresult.startLocations == NULL && cresult.endLocations==NULL) {
                //start
                start_end.push_back(NA_INTEGER);
                //end
                start_end.push_back(NA_INTEGER);
            }
            if (cresult.startLocations != NULL && cresult.endLocations==NULL) {
                //start
                start_end.push_back(cresult.startLocations[i]);
                //end
                start_end.push_back(NA_INTEGER);
            } else if (cresult.startLocations == NULL && cresult.endLocations!=NULL) {
                //start
                start_end.push_back(NA_INTEGER);
                //end
                start_end.push_back(cresult.endLocations[i]);
            } else {
                //start
                start_end.push_back(cresult.startLocations[i]);
                //end
            ...
            locations.push_back(start_end);
            start_end = {}; // reset to empty vector
        }
    }

    finalresult["locations"] = locations;  

    // deallocation
    if (additionalEqualities) {
        free(additionalEqualities);
    }
...
void edlib_export_functions(DllInfo* dll) {
    R_RegisterCCallable("edlibR", "edlibNewAlignConfig", (DL_FUNC)&edlibNewAlignConfig);
    R_RegisterCCallable("edlibR", "edlibDefaultAlignConfig", (DL_FUNC)&edlibDefaultAlignConfig);
    R_RegisterCCallable("edlibR", "edlibFreeAlignResult", (DL_FUNC)&edlibFreeAlignResult);
    R_RegisterCCallable("edlibR", "edlibAlign", (DL_FUNC)&edlibAlign);
    R_RegisterCCallable("edlibR", "edlibAlignmentToCigar", (DL_FUNC)&edlibAlignmentToCigar);
}
